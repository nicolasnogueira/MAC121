...
//
// Este modulo implementa a interface ordenacao.h
//
// As funcoes estaticas utilizadas aqui foram retiradas do sitio
...
static void mergesortS (int p, int r, int v[]) {
    if (p < r - 1) {
        int q = (p + r)/2;
        mergesortS (p, q, v);
        mergesortS (q, r, v);
...
static void insercaoS (int n, int v[]) {
    int i, j, x;
    for (j = 1; j < n; ++j) {
        x = v[j];
...
static void quicksortS (int v[], int p, int r) {
    int j;                          
    if (p < r) {
        j = separa (v, p, r);
        quicksortS (v, p, j - 1);
